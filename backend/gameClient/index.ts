...
// Her mod için event listener'ları ekleyelim
function initGameModeListeners(): void {
    // Custom Mode
...
document.addEventListener('DOMContentLoaded', initGameModeListeners);
...
document.getElementById('custom-create-btn')?.addEventListener('click', function() {
    ...
    const gameType = gameTypeElement.value as GameType;
    ...
    switch (gameType) {
        ...
        case 'co-op':
            ...
            break;
        default:
            showNotification('Geçersiz oyun türü!', 'error');
            return;
    }

    // Elemet null kontrolü
    const paddleHeightEl = document.getElementById('paddle-height') as HTMLInputElement;
    const ballRadiusEl = document.getElementById('ball-radius') as HTMLInputElement;
    const cornerBoostEl = document.getElementById('corner-boost') as HTMLInputElement;
    const winningScoreEl = document.getElementById('winning-score') as HTMLInputElement;
...
document.getElementById('ai-start-btn')?.addEventListener('click', function() {
    // Difficulty seçimini al (null safety ile)
    const difficultyElement = document.querySelector('input[name="ai-difficulty"]:checked') as HTMLInputElement;
    ...
    // Veri hazırlama
    const data: AIGameData = {
        name: `${currentUserName}'s AI Room`,
        gameMode: "ai",
        gameSettings: {
            ...gameConfig.gameSettings,
            paddleHeight: parseInt((document.getElementById('ai-paddle-height') as HTMLInputElement).value, 10),
            ballRadius: parseInt((document.getElementById('ai-ball-radius') as HTMLInputElement).value, 10),
            ballSpeedIncrease: parseFloat((document.getElementById('ai-corner-boost') as HTMLInputElement).value),
            maxScore: parseInt((document.getElementById('ai-winning-score') as HTMLInputElement).value, 10)
        },
        aiSettings: {
            difficulty: difficultyElement.value as AISettings['difficulty'],
        }
    };
    ...
    if (difficulty === 'custom') {
        data.aiSettings = {
            ...data.aiSettings,
            reactionSpeed: parseInt((document.getElementById('ai-reaction-speed') as HTMLInputElement).value, 10),
            predictionAccuracy: parseInt((document.getElementById('ai-prediction-accuracy') as HTMLInputElement).value, 10),
            generalAccuracy: parseInt((document.getElementById('ai-general-accuracy') as HTMLInputElement).value, 10),
            learningSpeed: parseInt((document.getElementById('ai-learning-speed') as HTMLInputElement).value, 10),
            preparationDistance: parseInt((document.getElementById('ai-preparation-distance') as HTMLInputElement).value, 10),
            freezeDistance: parseInt((document.getElementById('ai-freeze-distance') as HTMLInputElement).value, 10),
            targetWinRate: parseInt((document.getElementById('ai-target-win-rate') as HTMLInputElement).value, 10),
            fairnessLevel: parseInt((document.getElementById('ai-fairness-level') as HTMLInputElement).value, 10),
            maxConsecutiveWins: parseInt((document.getElementById('ai-max-consecutive-wins') as HTMLInputElement).value, 10),
            rageMode: (document.getElementById('ai-rage-mode') as HTMLInputElement).checked,
            fatigueSystem: (document.getElementById('ai-fatigue-system') as HTMLInputElement).checked,
            focusMode: (document.getElementById('ai-focus-mode') as HTMLInputElement).checked,
            adaptiveDifficulty: (document.getElementById('ai-adaptive-difficulty') as HTMLInputElement).checked
        };
    }

    // Global değişkeni TypeScript ile uyumlu hale getir
    currentGameMode = 'ai';

    // Daha önce tartıştığımız güvenli socket gönderme yaklaşımını kullan
    if (roomSocket) {
        roomSocket.send("create", data);
...
document.getElementById('tournament-create-btn')?.addEventListener('click', function() {
    ...
    const tournamentSize = tournamentSizeElement.value as TournamentSize;
    ...
    if (tournamentSize === 'custom') {
        ...
        maxPlayers = parseInt(customSizeElement.value, 10);
        ...
        // Bitwise operatör ile 2'nin kuvveti kontrolü
        if ((maxPlayers & (maxPlayers - 1)) !== 0) {
            showNotification('Kişi sayısı 2\'nin kuvveti olmalıdır! (4, 8, 16, 32...)', 'error');
            return;
        }
    } else {
        // Standart boyutlar için direkt parse et
        maxPlayers = parseInt(tournamentSize, 10);
    }

    // Tournament data interface tanımı
    interface TournamentCreateData {
        ...
        [key: string]: any; // gameConfig için genişletilmiş tip
    }
...
document.getElementById('start-tournament-btn')?.addEventListener('click', function() {
    // Type assertion ile button elementini al
    const startButton = this as HTMLButtonElement;

    // Butonu devre dışı bırak ve yükleniyor göster
    startButton.disabled = true;
    startButton.innerHTML = '<div class="loading"></div> Turnuva Başlatılıyor...';
...
// Slider display update fonksiyonu
function updateAISliderDisplay(
    sliderId: string, 
    displayId: string, 
    formatter?: (value: string) => string
): void {
...
// AI Game Sliders
updateAISliderDisplay('ai-paddle-height', 'ai-paddle-height-value', (value) => `${value}px`);
updateAISliderDisplay('ai-ball-radius', 'ai-ball-radius-value', (value) => `${value}px`);
updateAISliderDisplay('ai-corner-boost', 'ai-corner-boost-value', (value) => `${parseFloat(value).toFixed(1)}x`);

// AI Custom Settings Sliders
function setupAICustomSliderDisplay(
    sliderId: string, 
    displayId: string, 
    formatter?: (value: string) => string
): void {
...
// Standart sliderları ayarla
aiCustomSliderConfigs.forEach(config => 
    setupAICustomSliderDisplay(config.sliderId, config.displayId)
);

// Özel formatör gereken sliderlar
setupAICustomSliderDisplay(
    'ai-target-win-rate', 
    'ai-target-win-rate-value', 
...
const settingsPanelsNew = document.querySelectorAll<HTMLElement>('.settings-panel');

settingsPanelsNew.forEach(panel => {
...
if (nextRoundBtn) {
    nextRoundBtn.addEventListener('click', () => {
        ...
        nextRoundBtn.innerHTML = '<div class="loading"></div> Starting Next Round...';
...
// Final Round butonuna event listener ekle
const finalRoundBtn2 = document.getElementById('final-round-btn') as HTMLButtonElement | null;
if (finalRoundBtn2) {
    finalRoundBtn2.addEventListener('click', () => {
        finalRoundBtn2.disabled = true;
        finalRoundBtn2.innerHTML = '<div class="loading"></div> Starting Final Round...';
        ...
        setTimeout(() => {
            if (finalRoundBtn2.disabled) {
                finalRoundBtn2.disabled = false;
                finalRoundBtn2.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="20" height="20" fill="white" style="margin-right: 8px;">
                        <path d="M400 0L176 0c-26.5 0-48.1 21.8-47.1 48.2c.2 5.3 .4 10.6 .7 15.8L24 64C10.7 64 0 74.7 0 88c0 92.6 33.5 157 78.5 200.7c44.3 43.1 98.3 64.8 138.1 75.8c23.4 6.5 39.4 26 39.4 45.6c0 20.9-17 37.9-37.9 37.9L192 448c-17.7 0-32 14.3-32 32s14.3 32 32 32l192 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-26.1 0C337 448 320 431 320 410.1c0-19.6 15.9-39.2 39.4-45.6c39.9-11 93.9-32.7 138.2-75.8C542.5 245 576 180.6 576 88c0-13.3-10.7-24-24-24L446.4 64c.3-5.2 .5-10.4 .7-15.8C448.1 21.8 426.5 0 400 0zM48.9 112l84.4 0c9.1 90.1 29.2 150.3 51.9 190.6c-24.9-11-50.8-26.5-73.2-48.3c-32-31.1-58-76-63-142.3zM464.1 254.3c-22.4 21.8-48.3 37.3-73.2 48.3c22.7-40.3 42.8-100.5 51.9-190.6l84.4 0c-5.1 66.3-31.1 111.2-63 142.3z"/>
                    </svg>
                    <span>Start Final Round</span>
